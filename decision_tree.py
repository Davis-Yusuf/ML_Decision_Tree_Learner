...
class DecisionTreeLearner:
    ...
    def decision_tree_learning(self, examples, attrs, parent=None, parent_examples=()):
        ...
        if len(examples) == 0:
            return DecisionLeaf(result=self.plurality_value(parent_examples), distribution=self.count_targets(examples), parent=parent)
        elif self.all_same_class(examples):
            return DecisionLeaf(result=examples[0][self.dataset.target], distribution=self.count_targets(examples), parent=parent)
        elif len(attrs) == 0:
            return DecisionLeaf(result=self.plurality_value(examples), distribution=self.count_targets(examples), parent=parent)
        else:
            a = self.choose_attribute(attrs=attrs, examples=examples)
            t = DecisionFork(attr=a, distribution=self.count_targets(examples), attr_name=self.dataset.attr_names[a], parent=parent)
            split_a = self.split_by(attr=a, examples=examples)
            for cat, ex in split_a:
                subtree = self.decision_tree_learning(examples=ex, attrs=remove_all(a, attrs), parent=t, parent_examples=examples)
    ...
    def choose_attribute(self, attrs, examples):
        ...
        max_gain_attr = 0
        for attr in attrs:
            inf_gain = self.information_gain(attr=attr, examples=examples)
    ...
    def information_gain(self, attr, examples):
        ...
        entropy = self.information_per_class(examples)  # target variable entropy
        remainder = 0
        total_ele_data = len(examples)  # p+n
        split_ex = self.split_by(attr, examples)
        # For all possible values in attribute e.g. hair is '0' and '1'
        for cat, ex in split_ex:
            num_exam = len(ex)  # pk+nk
            class_examples = self.split_by(self.dataset.target, ex)
            remainder_cat = 0
            # For all classes in attribute val e.g. hair is '0', look for all classes i.e. 'mammals', etc
            for class_cat, class_ex in class_examples:
                num_class = len(class_ex)  # pk
                if num_class == 0 or num_exam == num_class:
                    continue
                prob = num_class / num_exam
                log_prob = np.log2(prob)
                remainder_cat += log_prob * prob
            remainder = remainder + (num_exam / total_ele_data * remainder_cat)
    ...
    def information_content(cls, class_counts):
        ...
        norm_class_counts = normalize(class_counts)
        entropy = 0
        for p in norm_class_counts:
            if p != 0:
                entropy += (-p) * np.log2(p)
        return entropy
    ...
    def information_per_class(self, examples):
        ...
        targets = self.count_targets(examples)
        info = self.information_content(targets)
        return info
    ...
    def prune(self, p_value):
        ...
        while queue:
            s = queue.pop(0)
            if isinstance(s, DecisionLeaf):
                # print(f"visited leaf {s.result}")
                continue
            else:
                visited.append(s)
                # print(f"visited fork {s.attr_name}")
                for branchKey in s.branches.keys():
                    queue.append(s.branches[branchKey])
        value, similar = self.chi2test(p_value, fork=visited[-1])
        # if similar is True, then we prune else we ignore
        if similar:
            # convert Fork into Leaf
            pass
    ...
    def chi2test(self, p_value, fork):
        ...
        print("This is")
        print(fork.)
        ex = fork.predict(fork.)
        ppf = scipy.stats.chi2.ppf(1-p_value, self.dof)
        return None, True
